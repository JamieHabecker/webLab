...
.controller('DMVHomeController',['$scope','$routeParams','$timeout','$location','Locations', '$cookieStore','NoticesFactory','LocationFactory',function($scope,$routeParams,$timeout,$location,Locations, $cookieStore,NoticesFactory,LocationFactory){

home($scope,$routeParams,$timeout,$location,Locations, $cookieStore,NoticesFactory)
$scope.tab= function(x){
	console.log($scope.activePath + "" + x)
	if(x === "Notices"){
		$scope.activePath= "/Notices"
		NoticesController($scope,NoticesFactory,$location,x)
	}
	if(x === "LOcations"){
...
function NoticesController($scope,NoticesFactory,$location){
...
.service('Locations', function($http){
	var Message = {
		async: function() {
			var promise = $http.get('http://dmvnew/apps/dmvnowinterface/DMVNowInterface.aspx?function=locations', {cache:false}).then(function (response) {
				return{
...
.factory('LocationFactory',['$scope','$location','Locations','$cookieStore', function($scope,$location,Locations,$cookieStore){
	var latLng;
	$scope.notMobile = "notMobile"
	Locations.async().then(function(d) {
		var goTo = $scope.LocationDetails = d.allDetails;
		$scope.colors = d.allOffices();
	...
	}else if($cookieStore.get("gotLocation") === "true"){
		var lat = $cookieStore.get("userLat");
		var lng = $cookieStore.get("userLng");
		latLng = new google.maps.LatLng(lat,lng);
	}else if(navigator.geolocation && $cookieStore.get("gotLocation") !== "true"){
		$cookieStore.remove("gotLocation");
	...
	function positionCallback(position){
		...
		$cookieStore.put("gotLocation","true");
		$cookieStore.put("userLat",position.coords.latitude);
		$cookieStore.put("userLng",position.coords.longitude);
	...
	function drawMap($scope){
		...
		angular.forEach($scope.LocationDetails, function(value, key) {
			...
			if (value.OpenClosed === "O") {
				boxText.innerHTML = "<a  href='#/Office:" + value.ID + "'>" + value.OFFICENAME + "<br/>" + "Current wait: " + value.WAIT;
				+"</a>";
			} else if (value.OpenClosed === "C") {
				boxText.innerHTML = "<a  href='#/Office:" + value.ID + "'>" + value.OFFICENAME + "<br/>Currently Closed</a>";
			}else if (value.OpenClosed === "E") {
				boxText.innerHTML = "<a  href='#/Office:" + value.ID + "'>" + value.OFFICENAME + "<br/>Closed</a>";
			}
			var myOptions = {
				content : boxText,
				disableAutoPan : true,
		...
		google.maps.event.addListener($scope.myMap, 'dragstart', function() {
		});
	};
}])
